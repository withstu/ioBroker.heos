...
class HeosPlayer {
	...
	async connect(){
		...
		this.connect_timeout = setTimeout(async () => {
			await that.heos.setStateAsync(that.state_path + 'connected', true, true);
			that.connected = true;
			that.autoPlay();
		}, 5000);
	}
	...
	detectHeosResponseTimeout(){
		...
		if(this.state_simple && this.heos.getUptime(this.ip) >= 5) {
			this.respond_timeout = setTimeout(() => {
				that.setError('HEOS is not responding as expected. Request timed out. Update play state.', ERROR_CODES.Timeout);
				that.sendCommand('get_play_state');
			}, 60000);
		}
	}
	...
	checkHealth(){
		if(this.heos.getUptime(this.ip) >= 5) {
			//Reboots
			if(this.heos.getReboots(this.ip) >= 10){
			...
			//Leader Failures
			if(this.heos.getLeaderFailures(this.ip) >= 10){
				if(this.heos.config.rebootOnFailure === true) {
			...
			//Timeout Failures
			if(this.heos.getFailuresByCode(this.ip,ERROR_CODES.Timeout) >= 2){
				if(this.heos.config.rebootOnFailure === true) {
			...
			//Upnp Failures
			if(this.heos.getFailuresByCode(this.ip,ERROR_CODES.Upnp) >= 10){
				if(this.heos.config.rebootOnFailure === true) {
			...
			//Overall Failures
			if(this.heos.getFailures(this.ip) > 20){
				if(this.heos.config.rebootOnFailure === true) {
					...
					switch (cmd) {
						...
						case 'get_now_playing_media':
							...
							this.now_playing_update_timeout = setTimeout(async () => {
								this.logWarn('Update Now Playing');
								this.sendCommand('get_now_playing_media');
							}, 30000);
	...
	sendCommand(cmd) {
		...
		for (let c = 0; c < cmds.length; c++) {
			this.heos.queueMsg(this.commandToMsg(cmds[c]));
		}
	}
		...
		if(Object.keys(this.storedState).length != 0){
			...
			if(this.storedState.state == 'stop'){
				this.restore_timeout = setTimeout(() => {
					that.sendCommand('set_play_state&state=' + this.storedState.state);
				}, 2000);
			}
	...
	tts(fileName, volume){
		...
		this.tts_timeout = setTimeout(() => {
			that.logDebug('TTS timeout. Stop.', false);
			that.stopTTS();
		}, 30000);
	}
	...
	async stopTTS(){
		...
		if(this.tts_playing && !this.tts_stop_timeout){
			...
			this.tts_stop_timeout = setTimeout(async () => {
				that.logDebug('Stop TTS.', false);
				if(that.tts_timeout) {
					clearTimeout(that.tts_timeout);
					that.tts_timeout = undefined;
				}

				//Remove TTS From Queue
				if(that.tts_queue.length){
					that.tts_queue.sort(function(a, b){return b-a;}); //DESC
					for (let i = 0; i < that.tts_queue.length; i++) {
						that.sendCommand('remove_from_queue&qid=' + (that.tts_queue[i] + 1));
					}
					that.tts_queue = [];
				}
