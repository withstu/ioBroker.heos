...
class HeosPlayer {
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				...
				if(this.respond_timeout_counter < 2){
					this.setError('HEOS is not responding as expected. Update play state.');
					this.sendCommand('get_play_state');
				} else {
					if(this.heos.config.rebootOnFailure === true){
						this.setError('HEOS is not responding after a reasonable time. Reboot.');
						this.heos.reboot();
					} else {
	...
	async setError(last_error) {
		...
		if(this.heos.getFailures(this.ip) > 5){
			if(this.heos.config.rebootOnFailure === true) {
		...
		this.heos.setStateChanged(this.state_path + 'error', true, true, (error, id, notChanged) =>{
			if(!notChanged || (this.heos.getFailures(this.ip) > 0 && this.heos.getFailures(this.ip) < 5)){
				this.autoPlay();
			}
		});
		try {
				...
				case 'event':
					switch (cmd) {
						...
						case 'player_now_playing_changed':
							this.sendCommand('get_now_playing_media');
							this.detectHeosResponseTimeout(); //detect if player_now_playing_progress are coming in
							break;
