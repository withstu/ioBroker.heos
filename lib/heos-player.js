...
class HeosPlayer {
	...
	constructor(heos, player){
		...
		this.upnp_error_counter = 0;

		this.respond_timeout_counter = 0;
	...
	async initMetaData(player){
		...
		await this.heos.setObjectAsync(this.statePath + 'power', {
			type: 'state',
			common: {
				name: 'Power state',
		...
		await this.heos.setObjectAsync(this.statePath + 'version', {
			type: 'state',
			common: {
				name: 'Player version',
		...
		await this.heos.setObjectAsync(this.statePath + 'current_image_color_palette', {
			type: 'state',
			common: {
		...
		await this.heos.setObjectAsync(this.statePath + 'current_image_color_background', {
			type: 'state',
			common: {
		...
		await this.heos.setObjectAsync(this.statePath + 'current_image_color_foreground', {
			type: 'state',
			common: {
		...
		await this.heos.setObjectAsync(this.statePath + 'current_bitdepth', {
			type: 'state',
			common: {
		...
		await this.heos.setObjectAsync(this.statePath + 'reboot', {
			type: 'state',
			common: {
				name: 'Reboot player',
	...
	async connect(){
		this.heos.logInfo('connect HEOS player ' + this.name + ' (' + this.pid + ')', false);
		this.upnp_error_counter = 0;
		this.respond_timeout_counter = 0;
	...
	async disconnect(){
		this.heos.logInfo('disconnect HEOS player ' + this.name + ' (' + this.pid + ')', false);
		// activate forced autoplay for follower
		if(this.group_leader === true) {
			var pids = this.group_pid.split(',');
			for (var i = 0; i < pids.length; i++) {
				let pid = pids[i].trim();
				if(pid != this.pid){
					let player = this.heos.players[pid];
					if (player) {
						player.force_autoplay = true;
						this.heos.logDebug("activate force autoplay for: " + player.name, false);
					}
				}
			}
		}
	...
	muteRegex(payload){
		if(this.heos.config.muteOnRegex === true){
			...
			this.heos.logDebug("Test regex '" + regex + "' on " + payload + " with result: " + regex.test(payload), false);
			if(regex.test(payload) && this.muted === false){
				this.muted_regex = true;
				this.sendCommand('set_mute&state=on');
				this.heos.logInfo("Mute " + this.name + ", because regex matches", false);
			} else if(!regex.test(payload) && this.muted_regex === true){
				this.muted_regex = false;
				this.sendCommand('set_mute&state=off');
				this.heos.logInfo("Unmute " + this.name + ", because regex is not matching any more", false);
			}
		}
	}
			...
			if(this.auto_play === true) {
				if(this.connected === true
					...
					if(this.error === true || this.current_type.length == 0){
						this.heos.logInfo('auto play default music at ' + this.name, false);
						this.sendCommand(this.heos.config.autoPlayCmd);
					} else if(this.state_simple === false){
						this.heos.logInfo('auto play music at ' + this.name, false);
						this.sendCommand('set_play_state&state=play');
					} else {
						this.heos.logDebug('AutoPlay not started. Already playing. State:' + JSON.stringify(this, this.stringifyReplacer), false);
					}
				} else {
					this.heos.logDebug('AutoPlay not started on player ' + this.name + '. Player is not connected or muted.', false);
				}
			} else {
				this.heos.logDebug('AutoPlay is disabled on player ' + this.name, false);
			}
		} else {
			this.heos.logDebug('AutoPlay is disabled in configuration', false);
		}
	}
	...
	reboot(){
		if(this.heos.ip == this.ip){
			...
			if(!this.heos.reboot_ips.includes(this.ip)){
				this.heos.reboot_ips.push(this.ip);
			}
			this.heos.logWarn("rebooting player " + this.name + " (" + this.ip + ") requested. Needs to reconnect HEOS to the correct player first.", false);
			this.heos.reconnect();
		}
	}
	...
	detectHeosFailure(){
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				this.respond_timeout_counter += 1;
				if(this.respond_timeout_counter < 2){
					this.heos.logWarn("HEOS is not responding as expected. Update play state.", false)
					this.sendCommand('get_play_state');
				} else {
					if(this.heos.config.rebootOnFailure === true){
						this.heos.logWarn("HEOS is not responding after a reasonable time. Reboot.", false)
						this.heos.rebootAll();
					} else {
						this.heos.logWarn('Device failure detected. Activate "reboot on failure" in the configuration or reboot manually.', true);
					}
				}
			}, 30000);
		} else {
	...
	async setError(last_error) {
		...
		try {
			this.heos.logWarn('[setError] ' + last_error, false);
		...
		} catch (e) { this.heos.logError('[setLastError] ' + e.message, false); }
	}
	...
	async updateUpnpSongMeta(){
		...
		try{
			...
			if(result && result.TrackMetaData && result.TrackMetaData.res){
				let bitrate = '@_bitrate' in result.TrackMetaData.res ? Math.trunc(result.TrackMetaData.res['@_bitrate'] / 1000) : 0;
				let bitdepth = '@_bitsPerSample' in result.TrackMetaData.res ? result.TrackMetaData.res['@_bitsPerSample'] : 0;
				...
				this.heos.setState(this.statePath + "current_bitdepth", bitdepth, true);
				...
				this.heos.setState(this.statePath + "current_bitrate", "", true);
				this.heos.setState(this.statePath + "current_bitdepth", "", true);
				this.heos.setState(this.statePath + "current_sample_rate", "", true);
				this.heos.setState(this.statePath + "current_audio_format", "", true);
			}
		} catch(err){
			this.heos.logWarn('update upnp meta failed: ' + err, false)
			this.handleUpnpError();
		}
	}
	...
	async updateUpnpAllowedActions(){
		...
		try{
			...
			this.heos.logWarn('update upnp allowed actions failed: ' + err, false)
			this.handleUpnpError();
		}
	}
	...
	async getUpnpDevicePowerState(){
		let service = 'ACT'
		let action = 'GetDevicePowerState'
		let data = {
			InstanceID: 0
		}
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				var result = await this.upnp.sendCommand(service, action, data)
				this.power = (result.devicePower == "ON") ? true : false;
				this.heos.setState(this.statePath + "power", this.power, true);
			} catch(err){
				this.heos.logWarn('update device power state failed: ' + err, false)
			}
		} else {
			this.heos.logDebug('get device power state is not supported by player ' + this.name + ' (' + this.ip + ')', false);
		}
	}

	async setUpnpDevicePowerState(state){
		let service = 'ACT'
		let action = 'SetDevicePowerState'
		let data = {
			InstanceID: 0,
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				this.heos.logWarn('set device power state failed: ' + err, false);
			}
		} else {
			this.heos.logWarn('set device power state is not supported by player ' + this.name + ' (' + this.ip + ')', false);
		}
	}

	handleUpnpError(){
		this.upnp_error_counter += 1;
		if(this.upnp_error_counter >= 6){
			if(this.heos.config.rebootOnFailure === true){
				this.heos.logWarn("HEOS is not responding as expected. Reboot.", false)
				this.reboot();
			} else {
				this.heos.logWarn('Device failure detected. Activate "reboot on failure" in the configuration or reboot manually.', true);
			}
	...
	async parseResponse(jdata, jmsg, cmd_group, cmd) {
					...
					switch (cmd) {
						...
						case 'player_state_changed':
							...
							if(this.state_simple === false && this.force_autoplay === true) {
								this.force_autoplay = false;
								this.heos.logDebug("force autoplay", false)
								this.autoPlay();
							}
						...
						case 'player_volume_changed':
							...
							if(this.volume_max < jmsg.level){
								...
								this.heos.logInfo(this.name + ": Max volume reached. Reset to: " + volume, false);
							...
							if(jdata.payload.hasOwnProperty('sid') && jdata.payload.sid != 0){
								...
								if (jdata.payload.hasOwnProperty('image_url')) {
									...
									if(jdata.payload.image_url){
										try {
											let imageColors = await imageColorExtract(jdata.payload.image_url)
											this.heos.setState(this.statePath + "current_image_color_palette", JSON.stringify(imageColors.colorPalette), true);
											this.heos.setState(this.statePath + "current_image_color_background", imageColors.backgroundColor, true);
											this.heos.setState(this.statePath + "current_image_color_foreground", imageColors.foregroundColor, true);
										} catch(colorerr){
											this.heos.logDebug('Image color extraction failed');
											this.heos.setState(this.statePath + "current_image_color_palette", "", true);
											this.heos.setState(this.statePath + "current_image_color_background", "", true);
											this.heos.setState(this.statePath + "current_image_color_foreground", "", true);
										}
									} else {
										this.heos.setState(this.statePath + "current_image_color_palette", "", true);
										this.heos.setState(this.statePath + "current_image_color_background", "", true);
								...
								this.heos.logDebug("get_now_playing_media response ignored. Invalid sid.", false)
							}
							break;
						case 'get_queue':
							let queue = {};
		...
		} catch (err) { this.heos.logError('[parseResponse] ' + err.message, false); }
	}
	...
	async timeSeek(seconds){
		if(!this.allowed_actions.includes('Seek')){
			this.heos.logWarn('seek is currently not available', false)
		} else {
			...
			try{
				...
				this.heos.logWarn('seek failed:' + err, false)
			}
		}
	}
	...
	storeState(){
		...
		this.heos.getStates(this.statePath + "*", async (err, states) => {
			...
			this.heos.logDebug("Stored player state: " + JSON.stringify(this.storedState), false);
		})
	}
	
	restoreState(){
        if(Object.keys(this.storedState).length != 0){
			this.heos.logDebug("Restore player state: " + JSON.stringify(this.storedState), false);
	 ...
	 */
	tts(fileName, volume){
		this.heos.logDebug("TTS on " + this.name + ": " + fileName + " with volume " + volume, false);
		if(!this.tts_playing){
			this.storeState();
		}
		if(volume && this.volume_max < volume){
			this.heos.logInfo(this.name + ": TTS max volume reached. Reset to: " + this.volume_max, false);
			volume = this.volume_max;
		}
		...
		this.tts_timeout = setTimeout(() => {
			this.heos.logDebug("TTS timeout. Stop.", false)
			this.stopTTS();
		}, 30000);
	}
	...
	async stopTTS(){
		if(this.tts_playing && !this.tts_stop_timeout){
			...
			this.tts_stop_timeout = setTimeout(async () => {
				this.heos.logDebug("Stop TTS.", false)
				if(this.tts_timeout) {
					clearTimeout(this.tts_timeout);
					this.tts_timeout = undefined;
				}
