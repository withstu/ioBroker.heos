...
class HeosPlayer {
	...
	async setError(last_error) {
		...
		if(this.heos.getFailures(this.ip) >= 15){
			if(this.heos.config.rebootOnFailure === true) {
				this.heos.logWarn('Player ' + this.name + ' has to many failures. Reboot.');
				this.reboot();
			} else {
				this.heos.logWarn('Player ' + this.name + ' has to many failures. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.', true);
			}
		}
