...
import HeosUPnP from './heos-upnp.js';
import { imageColorExtract } from './image-color-extract.js';
...
class HeosPlayer {
    ...
    stringifyReplacer(key, value) {
        if (key == 'heos' || key.indexOf('timeout') > -1) {
            return undefined;
        }
        return value;
    }
    ...
    autoPlay() {
        if (this.heos.config.autoPlay === true) {
            if (this.auto_play === true) {
                if (this.connected === true && this.muted === false && this.isPlayerGroupLeader()) {
                    if (
                        (this.error === true && this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError) > 5) ||
                        this.current_type.length == 0
                    ) {
                    ...
                    this.logDebug(
                        `AutoPlay not started on player ${this.name}. Player is not connected, muted or not a leader.`,
                        false,
                    );
                }
            } else {
    ...
    reboot() {
        if (this.heos.getReboots(this.ip) < 20) {
            if (this.heos.ip == this.ip) {
                ...
                this.logWarn(
                    `rebooting player ${this.name} (${
                        this.ip
                    }) requested. Needs to reconnect HEOS to the correct player first.`,
                    false,
                );
                this.heos.reconnect();
            }
            ...
            this.logError(
                `adapter is not able to fix health of player ${this.name} (${
                    this.ip
                }) by reboots. Please turn off the power of the player and turn it on after some seconds.`,
                false,
            );
        }
    }
    ...
    detectHeosResponseTimeout() {
        ...
        if (this.state_simple && this.heos.getUptime(this.ip) >= 5) {
            this.respond_timeout = this.heos.setTimeout(() => {
                this.setError(
                    'HEOS is not responding as expected. Request timed out. Update play state.',
                    ERROR_CODES.Timeout,
                );
    ...
    checkHealth() {
        if (this.heos.getUptime(this.ip) >= 5) {
            //Reboots
            if (
                this.heos.getReboots(this.ip) >= 10 &&
                this.heos.getReboots(this.ip) < 20 &&
                this.heos.getReboots(this.ip) % 5 == 0
            ) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many reboots. Reboot all. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
            ...
            if (this.heos.getLeaderFailures(this.ip) >= 20) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many leader failures. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
            ...
            if (this.heos.getFailuresByCode(this.ip, ERROR_CODES.General) >= 10) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many general errors. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
            ...
            if (this.heos.getFailuresByCode(this.ip, ERROR_CODES.Timeout) >= 5) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many timeouts. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
            ...
            if (this.heos.getFailuresByCode(this.ip, ERROR_CODES.Upnp) >= 20) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many upnp failures. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
            ...
            if (this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError >= 10)) {
                if (this.heos.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        `[setError] Player ${
                            this.name
                        } has to many playback failures. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.`,
                        false,
                    );
                }
            }
    ...
    async setError(message, code) {
        ...
        this.heos.setStateChanged(`${this.state_path}error`, true, true, (error, id, notChanged) => {
            if (
                !notChanged ||
                (this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError) > 0 &&
                    this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError) < 3)
            ) {
                this.autoPlay();
            }
        });
        try {
            ...
            this.logError(`[setLastError] ${e}`, false);
        }
    }

    /**
     *
     * @param {boolean} deleteLastError deleteLastError
     */
     ...
     */
    async updateUpnp() {
        if (this.connected && (this.last_upnp_update == null || new Date().getTime() - this.last_upnp_update > 30)) {
            this.last_upnp_update = new Date().getTime();
            ...
            //await this.getUpnpRenderingControlState(); //Not data used
            //await this.getUpnpACTState(); //Device Power
            await this.getUpnpDevicePowerState();
            //await this.getUpnpGroupControlState(); //Not data used
            //await this.getUpnpZoneControlState(); //Not data used
            await this.getUpnpAVTransportState(); //Meta Data
        }
    }

    /**
     * getUpnpRenderingControlState
     */
    async getUpnpRenderingControlState() {
        ...
        if (this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)) {
            ...
            try {
                ...
                if (!(service in this.upnp_states)) {
                    this.upnp_states[service] = {};
                }
    ...
    async getUpnpACTState() {
        ...
        if (this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)) {
            ...
            if (state && state.DevicePower && state.DevicePower['@_val']) {
    ...
    async getUpnpAVTransportState() {
        ...
        if (this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)) {
            ...
            //Track Meta
            if (leader_state && leader_state.CurrentTrackMetaData && leader_state.CurrentTrackMetaData['@_val']) {
                ...
                let format = '';
                if (meta.desc) {
                    if (Array.isArray(meta.desc)) {
                        for (const desc of meta.desc) {
                            if (desc['@_id'] == 'audioFormat') {
                                format = desc.value;
                            }
                        }
                    } else if (meta.desc['@_id'] == 'audioFormat') {
            ...
            if (state && state.CurrentTransportActions && state.CurrentTransportActions['@_val']) {
    ...
    /**
     * getUpnpDevicePowerState
     */
    async getUpnpDevicePowerState() {
        const service = 'ACT';
        const action = 'GetDevicePowerState';
        const data = {
            InstanceID: 0,
        };
        if (this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)) {
            ...
            try {
                ...
                if (this.isPlayerGroupLeader()) {
                    state = leader_state = this.upnp_states[service]['devicePower'] = result['devicePower'];
                } else {
                    state = this.upnp_states[service]['devicePower'] = result['devicePower'];
                    const leader = this.heos.players[this.group_leader_pid];
                    if (leader && leader.upnp_states && service in leader.upnp_states) {
                        leader_state = leader.upnp_states[service];
                    } else {
                ...
                if (err.code != 'ECONNABORTED') {
                    this.heos.raiseFailures(this.ip, ERROR_CODES.Upnp);
                    this.logWarn(`[upnp] get device power state failed: ${err}`, false);
                } else {
                    this.logDebug(`[upnp] get device power state failed: ${err}`, false);
                }
            }
            ...
            this.logDebug(`[upnp] get device power state is not supported by player ${this.name} (${this.ip})`, false);
        }
    }
     ...
     * @param {*} jdata jdata
     * @param {*} jmsg jmsg
     * @param {string} cmd_group cmd_group
    ...
    async parseResponse(jdata, jmsg, cmd_group, cmd) {
        ...
        try {
                    ...
                    switch (cmd) {
                        ...
                        case 'player_volume_changed': {
                            ...
                            if (this.volume_lock && this.volume != volume) {
                                ...
                                this.muted = newMuted;
                                this.heos.setStateChanged(
                                    `${this.state_path}muted`,
                                    newMuted,
                                    true,
                                    (error, id, notChanged) => {
                                        if (!notChanged) {
                        ...
                        case 'player_now_playing_progress':
                            ...
                            this.heos.setState(
                                `${this.state_path}current_elapsed_s`,
                                this.toFormattedTime(jmsg.cur_pos / 1000, true),
                                true,
                            );
                            this.heos.setState(`${this.state_path}current_duration`, this.current_duration, true);
                            this.heos.setState(
                                `${this.state_path}current_duration_s`,
                                this.toFormattedTime(this.current_duration, true),
                                true,
                            );
                ...
                case 'player':
                    switch (cmd) {
                        ...
                        case 'get_volume':
                            this.heos.getState(`${this.state_path}volume`, async (err, state) => {
                                if (state == null || state == undefined || state.val != jmsg.level) {
                                    ...
                                    if (this.volume_lock && this.volume != volume) {
                                        this.logInfo(
                                            `${this.name}: Volume lock enabled. Reset to: ${this.volume}`,
                                            false,
                                        );
                                        this.sendCommand(`set_volume?level=${this.volume}`);
                                    } else {
                        ...
                        case 'get_now_playing_media':
                            ...
                            if (
                                Object.prototype.hasOwnProperty.call(jdata.payload, 'sid') &&
                                (jdata.payload.sid != 0 || JSON.stringify(jdata.payload).includes('tidal.com'))
                            ) {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'sid')) {
                                    ...
                                    if (source) {
                                        ...
                                        this.heos.setState(
                                            `${this.state_path}current_source_image_url`,
                                            source.image_url,
                                            true,
                                        );
                                    } else {
                                        this.heos.getState(`sources.${jdata.payload.sid}.name`, async (err, state) => {
                                            if (state && state.val && !state.notExist) {
                                                this.heos.setState(
                                                    `${this.state_path}current_source_name`,
                                                    `${state.val}`,
                                                    true,
                                                );
                                            } else {
                                        ...
                                        this.heos.getState(
                                            `sources.${jdata.payload.sid}.image_url`,
                                            async (err, state) => {
                                                if (state && state.val && !state.notExist) {
                                                    this.heos.setState(
                                                        `${this.state_path}current_source_image_url`,
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'type')) {
                                    ...
                                    if (jdata.payload.type == 'station' && jdata.payload.station !== undefined) {
                                        this.heos.setState(
                                            `${this.state_path}current_station`,
                                            jdata.payload.station,
                                            true,
                                        );
                                    } else {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'song')) {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'album')) {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'album_id')) {
                                    this.heos.setState(
                                        `${this.state_path}current_album_id`,
                                        jdata.payload.album_id,
                                        true,
                                    );
                                } else {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'artist')) {
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'image_url')) {
                                    this.heos.setState(
                                        `${this.state_path}current_image_url`,
                                        jdata.payload.image_url,
                                        true,
                                    );
                                    ...
                                    if (jdata.payload.image_url) {
                                        try {
                                            ...
                                            this.heos.setState(
                                                `${this.state_path}current_image_color_palette`,
                                                JSON.stringify(imageColors.colorPalette),
                                                true,
                                            );
                                            this.heos.setState(
                                                `${this.state_path}current_image_color_background`,
                                                imageColors.backgroundColor,
                                                true,
                                            );
                                            this.heos.setState(
                                                `${this.state_path}current_image_color_foreground`,
                                ...
                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'mid')) {
                                    this.heos.setState(`${this.state_path}current_mid`, jdata.payload.mid, true);
                                } else {
                                    this.heos.setState(`${this.state_path}current_mid`, '', true);
                                }

                                if (Object.prototype.hasOwnProperty.call(jdata.payload, 'qid')) {
                                    this.heos.setStateChanged(
                                        `${this.state_path}current_qid`,
                                        jdata.payload.qid,
                                        true,
                                        (error, id, notChanged) => {
                                            if (!notChanged) {
                                                ...
                                                if (jdata.payload.qid) {
                                                    ...
                                                    if (
                                                        this.tts_playing &&
                                                        Object.prototype.hasOwnProperty.call(jdata.payload, 'song') &&
            ...
            this.logError(`[parseResponse] ${err}`, false);
        }
    }
     ...
     *
     * @param {string} cmd cmd
     * @param ignore_duplicate_check ignore_duplicate_check
     */
    ...
    restoreState() {
        if (Object.keys(this.storedState).length != 0) {
            ...
            this.sendCommand(
                `set_play_mode?repeat=${this.getRepeatStateName(this.storedState.repeat)}&shuffle=${
                    this.storedState.shuffle === true ? 'on' : 'off'
                }`,
            );
            ...
            if (this.storedState.current_type == 'station') {
                ...
                switch (this.storedState.current_sid) {
                    ...
                    case 3:
                        this.sendCommand(
                            `play_stream?sid=${this.storedState.current_sid}&mid=${this.storedState.current_album_id}`,
                        );
                        break;
                    //Amazon
                    case 13:
                        this.sendCommand(
                            `play_stream?sid=${this.storedState.current_sid}&mid=${this.storedState.current_mid}`,
                        );
                        break;
                    //AUX
                    case 1027:
        ...
        if (this.tts_playing && !this.tts_stop_timeout) {
            ...
            this.tts_stop_timeout = setTimeout(async () => {
                ...
                if (this.tts_queue.length) {
                    this.tts_queue.sort(function (a, b) {
                        return b - a;
                    }); //DESC
