...
class HeosPlayer {
		...
		if(this.heos.config.autoPlay === true){
			if(this.auto_play === true) {
				if(this.connected === true
					&& this.muted === false){
					if((this.error === true && this.heos.getFailures(this.ip) > 8) || this.current_type.length == 0){
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				...
				if(this.respond_timeout_counter < 2){
					this.heos.raiseFailures(this.ip);
					...
					if(this.heos.config.rebootOnFailure === true){
						...
						this.heos.raiseLeaderFailures(this.heos.ip);
						this.heos.reboot();
					} else {
	...
	async setError(last_error) {
		this.error = true;
		this.heos.raiseFailures(this.ip);
		if(this.heos.getFailures(this.ip) >= 15){
			if(this.heos.config.rebootOnFailure === true) {
				this.logWarn('[setError] Player ' + this.name + ' has to many failures. Reboot.', false);
				this.reboot();
			} else {
				this.logWarn('[setError] Player ' + this.name + ' has to many failures. Reboot. Activate "reboot on failure" in the configuration or reboot manually the device.', false);
			}
		}
		this.heos.setStateChanged(this.state_path + 'error', true, true, (error, id, notChanged) =>{
			if((!notChanged || (this.heos.getFailures(this.ip) > 1 && this.heos.getFailures(this.ip) <= 10)) && this.isPlayerLeader()){
				this.autoPlay();
			}
		});
		try {
