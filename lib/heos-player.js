...
class HeosPlayer {
	...
	reboot(){
		if(this.heos.ip == this.ip){
			this.heos.log.warn("Rebooting player " + this.name);
			this.upnp_error_counter = 0;
			this.heos.reboot();
		} else {
			if(!this.heos.reboot_ips.includes(this.ip)){
			...
			this.heos.log.warn("Rebooting player " + this.name + " requested. Needs to reconnect HEOS to the correct player first.");
			this.heos.reconnect();
		}
	}
