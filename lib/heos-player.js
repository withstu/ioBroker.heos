...
class HeosPlayer {
			...
			for (var i = 0; i < pids.length; i++) {
				...
				if(pid != this.pid){
					...
					if (player) {
						...
						this.heos.log.debug("activate force autoplay for: " + player.name);
					}
				}
			}
		}
	...
	reboot(){
		if(this.heos.ip == this.ip){
			...
			this.heos.log.warn("rebooting player " + this.name + " (" + this.ip + ") requested. Needs to reconnect HEOS to the correct player first.");
			this.heos.reconnect();
		}
	}
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				...
				if(this.respond_timeout_counter < 2){
					this.heos.log.warn("HEOS is not responding as expected. Update play state.")
	...
	async getUpnpDevicePowerState(){
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				this.heos.log.warn('update device power state failed: ' + err)
			}
		} else {
			this.heos.log.debug('get device power state is not supported by player ' + this.name + ' (' + this.ip + ')');
		}
	}
	...
	async setUpnpDevicePowerState(state){
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				this.heos.log.warn('set device power state failed: ' + err);
			}
		} else {
			this.heos.log.warn('set device power state is not supported by player ' + this.name + ' (' + this.ip + ')');
		}
	}
					...
					switch (cmd) {
						...
						case 'player_state_changed':
							...
							if(this.state_simple === false && this.force_autoplay === true) {
								this.force_autoplay = false;
								this.heos.log.debug("force autoplay")
								this.autoPlay();
							}
