...
class HeosPlayer {
	...
	autoPlay(){
		if(this.heos.config.autoPlay === true){
			if(this.auto_play === true
				&& this.connected === true
				&& (this.state_simple === false || this.current_type.length == 0)
				&& this.muted === false){
					...
					switch (cmd) {
						...
						case 'player_volume_changed':
							...
							if(this.volume_max < jmsg.level){
								...
								this.heos.log.info("Max volume reached. Reset to: " + volume);
	...
	tts(fileName, volume){
		...
		if(volume && this.volume_max < volume){
			this.heos.log.info("TTS max volume reached. Reset to: " + this.volume_max);
			volume = this.volume_max;
		}
