...
class HeosPlayer {
	...
	detectHeosResponseTimeout(){
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				this.setError('HEOS is not responding as expected. Update play state.', ERROR_CODES.Timeout);
	...
	checkHealth(){
		//Timeout Failures
		if(this.heos.getFailuresByCode(this.ip,ERROR_CODES.Timeout) > 4){
	...
	async setError(message, code) {
		...
		this.heos.setStateChanged(this.state_path + 'error', true, true, (error, id, notChanged) =>{
			if(!notChanged || (this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError) > 0 && this.heos.getFailuresByCode(this.ip, ERROR_CODES.PlaybackError) < 10)){
				this.autoPlay();
			}
		});
		try {
