...
class HeosPlayer {
	...
	async initMetaData(player){
		...
		this.heos.getStateAsync(this.state_path + 'auto_play', (err, state) => {
			this.auto_play = state.val;
		});
		this.heos.getStateAsync(this.state_path + 'ignore_broadcast_cmd', (err, state) => {
			this.ignore_broadcast_cmd = state.val;
		});
		this.heos.getStateAsync(this.state_path + 'volume_limit', (err, state) => {
			this.volume_limit = state.val;
		});
	}
	...
	async connect(){
		...
		this.connect_timeout = setTimeout(async () => {
			await this.heos.setStateAsync(this.state_path + 'connected', true);
			this.connected = true;
			this.autoPlay();
		}, 5000);
	}
			...
			for (var i = 0; i < pids.length; i++) {
				...
				if(pid != this.pid){
					...
					if (player) {
						...
						this.logDebug("activate force autoplay for: " + player.name, false);
					}
				}
			}
		}
			...
			if(this.auto_play === true) {
				if(this.connected === true
					...
					if(this.error === true || this.current_type.length == 0){
						...
						this.logDebug('AutoPlay not started. Already playing. State:' + JSON.stringify(this, this.stringifyReplacer), false);
					}
				} else {
					this.logDebug('AutoPlay not started on player ' + this.name + '. Player is not connected or muted.', false);
				}
			} else {
		...
		if(this.state_simple) {
			this.respond_timeout = setTimeout(() => {
				...
				if(this.respond_timeout_counter < 2){
					this.logWarn("HEOS is not responding as expected. Update play state.", false)
					this.sendCommand('get_play_state');
				} else {
					if(this.heos.config.rebootOnFailure === true){
						this.logWarn("HEOS is not responding after a reasonable time. Reboot.", false)
						this.heos.rebootAll();
					} else {
	...
	async setError(last_error) {
		...
		this.heos.setStateChanged(this.state_path + 'error', true, true, (error, id, notChanged) =>{
			if(!notChanged && this.isPlayerLeader()){
				this.autoPlay();
			};
		});
		try {
			this.logWarn('[setError] ' + last_error, false);
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				if(this.isPlayerLeader()){
					...
					if(leader && service in leader.upnp_states){
				...
				this.logWarn('get ' + service + ' state failed: ' + err, false)
			}
		} else {
			this.logDebug('get ' + service + ' state is not supported by player ' + this.name + ' (' + this.ip + ')', false);
		}
	}
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				if(state
					...
					this.heos.setState(this.state_path + "power", "", true);
				}
			} catch(err){
				this.logWarn('get ' + service + ' state failed: ' + err, false)
			}
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				if(leader_state 
					...
					let meta = leader_state.CurrentTrackMetaData["@_val"];
					if(leader_state.CurrentTrackMetaData["@_val"].res) {
						let bitrate = '@_bitrate' in meta.res ? Math.trunc(meta.res['@_bitrate'] / 1000) : 0;
						let bitdepth = '@_bitsPerSample' in meta.res ? meta.res['@_bitsPerSample'] : 0;
						let sample_rate = '@_sampleFrequency' in meta.res ? meta.res['@_sampleFrequency'] / 1000 : 0;
						...
						this.heos.setState(this.state_path + "current_bitrate", "", true);
						this.heos.setState(this.state_path + "current_bitdepth", "", true);
						this.heos.setState(this.state_path + "current_sample_rate", "", true);
					}
					...
					this.heos.setState(this.state_path + "current_bitdepth", "", true);
					this.heos.setState(this.state_path + "current_sample_rate", "", true);
					this.heos.setState(this.state_path + "current_audio_format", "", true);
				}

				if(state
					&& state.CurrentTransportActions
					&& state.CurrentTransportActions["@_val"]){
					let actions = state.CurrentTransportActions["@_val"];
	...
	async setUpnpDevicePowerState(state){
		...
		if(this.upnp.hasService(service) && this.upnp.hasServiceAction(service, action)){
			try{
				...
				this.logWarn('set device power state failed: ' + err, false);
			}
		} else {
			this.logWarn('set device power state is not supported by player ' + this.name + ' (' + this.ip + ')', false);
		}
	}
	...
	async parseResponse(jdata, jmsg, cmd_group, cmd) {
				...
				case 'event':
					switch (cmd) {
						...
						case 'player_volume_changed':
							...
							if(this.volume_limit < jmsg.level){
						...
						case 'repeat_mode_changed':
							this.heos.setState(this.state_path + "repeat", jmsg.repeat, true);
							break;
						...
						case 'get_now_playing_media':
							...
							if(jdata.payload.hasOwnProperty('sid') && jdata.payload.sid != 0){
								...
								if (jdata.payload.hasOwnProperty('sid')) {
									...
									this.heos.setState(this.state_path + "current_sid", "", true);
									this.heos.setState(this.state_path + "current_source_name", "", true);
									this.heos.setState(this.state_path + "current_source_image_url", "", true);
								}
								...
								if (jdata.payload.hasOwnProperty('qid')) {
									...
									this.heos.setStateChanged(this.state_path + "current_qid", "", true, (error, id, notChanged) =>{
		...
		} catch (err) { this.logError('[parseResponse] ' + err.message, false); }
	}
