...
const extractImageColor = function (palette) {
    ...
    for (const color in palette) {
        if (Object.prototype.hasOwnProperty.call(palette, color) && palette[color]) {
            const obj = {};
            obj.rgb = palette[color].getRgb();
            obj.hex = palette[color].getHex();
            obj.hsl = palette[color].getHsl();
    ...
    const sortedPalette = Object.values(palette).sort((colorA, colorB) => colorB.population - colorA.population);
    const backgroundColor = sortedPalette[0];
    let foregroundColor;

    const contrastRatios = {};
    const approvedContrastRatio = color => {
        if (!Object.prototype.hasOwnProperty.call(contrastRatios, color.getHex())) {
    ...
    for (let i = 1; i < sortedPalette.length && foregroundColor === undefined; i++) {
        ...
        const currentColor = sortedPalette[i];
        for (let j = i + 1; j < sortedPalette.length; j++) {
            const compareColor = sortedPalette[j];
            const diffScore =
...
export const imageColorExtract = (url, downsampleColors = 16) =>
    new Vibrant(url, {
