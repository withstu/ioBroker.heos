...
class Heos extends utils.Adapter {
    ...
    resetVars(initMode) {
        ...
        this.source_map = {
            ...
            1024: {
                ...
                image_url:
                    'https://production.ws.skyegloup.com/media/images/service/logos/musicsource_logo_servers.png',
            },
            1025: {
                name: 'HEOS Playlists ',
                image_url:
                    'https://production.ws.skyegloup.com/media/images/service/logos/musicsource_logo_playlists.png',
            },
            1026: {
                name: 'HEOS History',
                image_url:
                    'https://production.ws.skyegloup.com/media/images/service/logos/musicsource_logo_history.png',
            },
            1027: {
            ...
            1028: {
                name: 'HEOS Favorites',
                image_url:
                    'https://production.ws.skyegloup.com/media/images/service/logos/musicsource_logo_favorites.png',
            },
        };
    ...
    onStateChange(_id, state) {
        ...
        this.logDebug(
            `State change - ID: ${_id} | DCS: ${JSON.stringify(id)} | State: ${JSON.stringify(state)}`,
            false,
        );
        ...
        if (id) {
            ...
            if (id.device === 'command') {
                ...
                id.channel === '1025' &&
                id.state &&
                fullId[fullId.length - 1] === 'play'
            ) {
                this.sendCommandToAllPlayers(
                    `add_to_queue?sid=1025&aid=${this.config.queueMode}&cid=${id.state}`,
                    true,
                );
            } else if (
                id.device === 'sources' &&
    ...
    async onNodeSSDPResponse(headers, statusCode, rinfo) {
        ...
        try {
            ...
            this.logError(`[onNodeSSDPResponse] ${err}`, false);
        }
    }
    ...
    onData(data) {
        ...
        try {
            ...
            this.logError(`[onData] ${err}`, false);
        }
    }
    ...
    async parseResponse(response) {
        try {
                        ...
                        case 'get_music_sources':
                            if (Object.prototype.hasOwnProperty.call(jdata, 'payload')) {
                                ...
                                for (i = 0; i < jdata.payload.length; i++) {
                                    ...
                                    const source = this.mapBrowseCmd(
                                        browse,
                                        decodeURIComponent(payload.name),
                                        payload.image_url,
                            ...
                            if (Object.prototype.hasOwnProperty.call(jdata, 'payload')) {
                                ...
                                if (playable) {
                                    browseResult['payload'].push({
                                        ...
                                        available: true,
                                        commands: {
                                            play: `scope/add_to_queue?sid=${sid}&cid=${jmsg.cid}&aid=${
                                    ...
                                    case 1025: {
                                        ...
                                        for (i = 0; i < jdata.payload.length; i++) {
                                            ...
                                            browseResult['payload'].push({
                                                ...
                                                available: true,
                                                commands: {
                                                    play: `scope/add_to_queue?sid=1025&aid=${
                                                        this.config.queueMode
                                                    }&cid=${payload.cid}`,
                                                },
                                            });
                                                    ...
                                                    if (states[id] && states[id].val) {
                                                        ...
                                                        if (state == 'cid') {
                                                            if (!playlists.includes(states[id].val)) {
                                                                this.logWarn(
                                                                    `deleting playlist: ${states[id].val}`,
                                                                    false,
                                                                );
            ...
            this.logError(`[parseResponse] ${err}\n ${response}`);
        }
    }
    ...
    sendCommandToAllPlayers(cmd, leaderOnly) {
        if (this.state == STATES.Connected) {
            for (const pid in this.players) {
                ...
                if (
                    player &&
                    player.ignore_broadcast_cmd === false &&
                    (!leaderOnly || (leaderOnly && player.isPlayerGroupLeader()))
                ) {
    ...
    async startPlayers(payload) {
        try {
            ...
            for (let i = 0; i < payload.length; i++) {
                ...
                if (Object.prototype.hasOwnProperty.call(player, 'ip') && player.ip != '127.0.0.1') {
                    foundPlayerIps.push(player.ip);
                }
                if (!player.name || !Object.prototype.hasOwnProperty.call(player, 'ip') || player.ip == '127.0.0.1') {
                    ...
                    if (this.start_players_errors > 4) {
                        ...
                        if (this.config.rebootOnFailure === true) {
                            ...
                            this.logWarn(
                                'Device failure detected. Activate "reboot on failure" in the configuration or reboot manually.',
                                true,
                            );
                        }
                    }
                    ...
                    if (!(pid in this.players)) {
                        ...
                        if (
                            this.preferred_player_ips.length > 0 &&
            ...
            for (const id in this.ssdp_player_ips) {
                ...
                if (this.getUptime(ip) >= 5 && !foundPlayerIps.includes(ip)) {
                    ...
                    this.logDebug(
                        `Connected Players: ${JSON.stringify(foundPlayerIps)} | Announced Players: ${JSON.stringify(
                            this.ssdp_player_ips,
                        )}`,
                        true,
                    );
            ...
            if (playerInFailState) {
                ...
                if (this.config.rebootOnFailure === true) {
                    ...
                    this.logWarn(
                        'Can\'t connect any players. Activate "reboot on failure" in the configuration or reboot manually.',
                        true,
                    );
                }
            } else {
    ...
    async setGroup(group) {
        if (Object.prototype.hasOwnProperty.call(group, 'pid')) {
            ...
            for (let i = 0; i < pids.length; i++) {
                ...
                const player = this.players[pid];
                if (player) {
                    await player.setGroupName(Object.prototype.hasOwnProperty.call(group, 'name') ? group.name : '');
            ...
            if (Object.prototype.hasOwnProperty.call(group, 'gid')) {
        ...
        if (typeof this.net_client == 'undefined' && this.state == STATES.Searching) {
            this.ssdp_search_timeout = setTimeout(() => {
                if (this.state == STATES.Searching && this.ssdp_player_ips.length > 0) {
                    ...
                    const minFailureIps = this.ssdp_player_ips.filter(element =>
                        this.getMinFailures().includes(element),
                    );
                    const minLeaderFailureIps = this.ssdp_player_ips.filter(element =>
                        this.getMinLeaderFailures().includes(element),
                    );
    ...
    async search() {
        try {
            ...
            if (this.reboot_ips.length > 0) {
                ...
                this.nodessdp_client.on('response', (headers, statusCode, rinfo) =>
                    this.onNodeSSDPResponse(headers, statusCode, rinfo),
                );
                this.nodessdp_client.on('error', error => {
                    this.nodessdp_client.close();
                    this.logError(`[nodessdp] ${error}`, false);
                });
    ...
    async reconnect() {
        if (this.state == STATES.Reconnecting || this.state == STATES.Disconnecting) {
            return;
        }
